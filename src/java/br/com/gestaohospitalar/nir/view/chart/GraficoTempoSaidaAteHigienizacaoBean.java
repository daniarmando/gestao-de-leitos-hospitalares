...
@ManagedBean
@RequestScoped
public class GraficoTempoSaidaAteHigienizacaoBean implements Serializable {
    ...
    /**
     * Creates a new instance of GraficoTempoSaidaAteHigienizacaoBean
     */
    public GraficoTempoSaidaAteHigienizacaoBean() {
    ...
    public void prepararGrafico() {
        ...
        if (PeriodoValidator.comparaDatas(this.dataInicial, this.dataFinal) == false) {
            ...
            if (this.leitos.isEmpty()) {
                ...
                graficoTempoSaidaAteHigienizacao();
            }
        }
    }

    /**
     * método que chama as rotinas para montar o gráfico de tempo de tempo saída
     * paciente até a higienização
     *
     */
    public void graficoTempoSaidaAteHigienizacao() {
        ...
        switch (this.modeloGrafico) {
            ...
            case 2: //gráfico de área
                ...
                layoutGraficoLinha("Diferença Tempo de Saída do Paciente até Início da Higienização", "Data saída", "Diferença em minutos");
                break;

            case 3: //gráfico de barra
                //montando o layout do gráfico de barra
                layoutGraficoBarra("Diferença Tempo de Saída do Paciente até Início da Higienização", "Data saída", "Diferença em minutos");
                break;
        }
    ...
    public void adicionarSerie(Integer idLeito, String rotulo) {
        ...
        //recebendo os valores do tempo de saída x higienização por leitos do período informado
        Map<Date, Double> valoresPorData = tempoSaidaAteHigienizacao(idLeito, this.dataInicial, this.dataFinal);
    ...
    /**
     * método que retorna os valores do tempo de saída x higienização 
     * por leitos do período informado
     *
     * @param idLeito
     * @param dataInicial
     * @param dataFinal
     * @return resultado
     */
    public Map<Date, Double> tempoSaidaAteHigienizacao(Integer idLeito, Date dataInicial, Date dataFinal) {
        ...
        //de dataSaida da internação (eixo x)
        //e tempo em minutos da diferença saída x higienização (eixo y)
        for (Estatisticas estatistica : this.listaEstatisticas) {

            //calculando diferença saída x higienização em minutos
            qtdDifTempoMinutos = estatistica.getTempoSaidaAteHigienizacao() / 60.0;

            //adicionando as informações ao map
            resultado.put(estatistica.getInternacao().getDataSaidaLeito(), qtdDifTempoMinutos);
